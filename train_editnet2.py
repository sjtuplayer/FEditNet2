...
from editnet2 import EditNet
from data import build_dataloader2, sample_data
from utils.parser2 import parse_args
...
def train(opt):
    ...
    if opt.ckpt is not None:
        editnet.load_networks_fine_tune()
        try:
    ...
    # build the dataset
    dataloader = build_dataloader2(opt, opt.seed)
    ...
    for i in pbar:
        ...
        if (i + start_iter) % opt.optim_freq == 0 and opt.learnable_weights:
        ...
        if (i + start_iter) % opt.print_freq == 0:
            ...
            for name, loss in sum_loss.items():
                logger.add_scalar(f'Loss/loss_{name}',
                                  loss / opt.print_freq,
                                  i + start_iter)
                Note = open('%s-%s_loss.txt' % (opt.attr1, opt.attr2), mode='a')  # write reward
                Note.write('step:%d loss_%s:%f\t' % (i + start_iter, name, loss / opt.print_freq))
                sum_loss[name] = 0.
            Note.write('\n')
            Note.close()
...
if __name__ == '__main__':
    args = parse_args()
    train(args)
#python3 train_editnet.py --attr1=xxx --attr2=xxx