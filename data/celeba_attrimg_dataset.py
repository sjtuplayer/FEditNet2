...
class AttrImgDataset0(data.Dataset):
    def __init__(self,
                 data_dir='./dataset/celeba-hq',
                 attr_dir='./dataset/list_attr_celeba-hq.txt',
                 attr_pairs=None,
                 transform=None
                 ):
...
class AttrImgDataset(data.Dataset):
    def __init__(self,
        ...
        for index in range(len(attr)):
            if  self.attr[index][self.attr_indices]==1:
                self.pos_ids.append(index)
            else:
                self.neg_ids.append(index)

    def __getitem__(self, index):
        if random.random()>0.5:
            index=index%len(self.pos_ids)
            index=self.pos_ids[index]
        else:
            index=index%len(self.neg_ids)
            index = self.neg_ids[index]
        img_path = self.img_paths[index % self.length]
        img = Image.open(img_path)
        if self.transform is not None:
            img = self.transform(img)
