...
from editnet3 import EditNet
from data import build_dataloader3, sample_data
from utils.parser3 import parse_args
...
def train(opt):
    ...
    # build the dataset
    dataloader = build_dataloader3(opt, opt.seed)
        ...
        if (i + start_iter) % opt.print_freq == 0:
            ...
            for name, loss in sum_loss.items():
                logger.add_scalar(f'Loss/loss_{name}',
                                  loss / opt.print_freq,
                                  i + start_iter)
                Note = open('output_loss.txt', mode='a')  # write reward
                Note.write('step:%d loss_%s:%f\n' % (i + start_iter, name, loss))
                Note.close()
                sum_loss[name] = 0.
...
if __name__ == '__main__':
    args = parse_args()
    train(args)
#python3 train_editnet.py --attr1=xxx --attr2=xxx --attr3=xxx