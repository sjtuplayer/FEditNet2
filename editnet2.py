...
from models.models import PatchSampleF, BoundaryGenerator2
from models.stylegan_model import Discriminator2, Generator
...
class EditNet():
    ...
    def __init__(self, opt):
        ...
        if not self.opt.finetune_D_backbone:
            self.optim_D = Adam(list(self.D.final_linear1.parameters())+list(self.D.final_linear2.parameters()),
    ...
    def data_dependent_initialize(self, data):
        ...
        if self.opt.ckpt is not None:
            ...
            for key in list(ckpt1['F'].keys()):
                ckpt1['F'][key] = (ckpt1['F'][key] + ckpt2['F'][key]) / 2
            self.F.load_state_dict(ckpt1['F'])
    ...
    def calc_nce_loss(self, _feat_src, _feat_tgt):
        ...
        for i, (f_q, f_k, crit) in enumerate(zip(feat_q_pool, feat_k_pool, self.criterionNCE)):
            loss = crit(f_q, f_k) * self.nce_weights[i]
            if self.learnable_weights:
    ...
    def backward_Gs(self):
        ...
        if self.opt.lambda_decouple!=0:
            decouple_loss = self.G.cal_decouple_loss().mean()
            decouple_loss = decouple_loss*self.opt.lambda_decouple
            self.loss_dict.update(decouple=decouple_loss)
