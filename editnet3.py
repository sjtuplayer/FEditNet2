...
from models.models import PatchSampleF, BoundaryGenerator3
from models.stylegan_model import Discriminator3, Generator
...
class EditNet():
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['d']
        # G losses
        self.loss_names += ['g', 'patchnce', 'idloss', 'l2loss']
        if opt.lambda_decouple != 0:
            self.loss_names += ['decouple1']
            self.loss_names += ['decouple2']
            self.loss_names += ['decouple3']
        if opt.lambda_cross != 0:
            self.loss_names += ['cross']
        # visuals
        ...
        print('load stylegan from %s' % self.opt.stylegan_ckpt)
        stylegan_ckpt = torch.load(self.opt.stylegan_ckpt)
        self.stylegan.load_state_dict(stylegan_ckpt['g_ema'])
        d_ckpt = stylegan_ckpt['d']
        # print(d_ckpt.keys())
        for key in list(d_ckpt.keys()):
            if 'final_linear' in key:
                key1 = key.replace('final_linear', 'final_linear1')
                key2 = key.replace('final_linear', 'final_linear2')
                key3 = key.replace('final_linear', 'final_linear3')
                d_ckpt[key1] = d_ckpt[key]
                d_ckpt[key2] = d_ckpt[key]
                d_ckpt[key3] = d_ckpt[key]
                d_ckpt.pop(key)

        self.D.load_state_dict(d_ckpt)
        ...
        if not self.opt.finetune_D_backbone:
            self.optim_D = Adam(list(self.D.final_linear1.parameters()) + list(self.D.final_linear2.parameters()) + list(self.D.final_linear3.parameters()),
    ...
    def run_G(self, label):
        ...
        syn_latent_edited = self.G(syn_latent, label,
    ...
    def backward_Ds(self, data):
        ...
        real_label = data['label'].detach()
        # label = (torch.rand(real_img.size(0)) > 0.5).int().detach()
        label = (torch.randint(0,3,(real_img.size(0),))).int().detach()

        fake_data = self.run_G(label)
        ...
        # D loss
        real_img_pred, _ = self.D(real_img, real_label, return_logits=True)
        syn_img_edited_pred = self.D(syn_img_edited, label)
    ...
    def backward_Gs(self):
        ...
        # forawrd
        # label = (torch.rand(self.batch_size) > 0.5).int()
        label = (torch.randint(0, 3, (self.batch_size,))).int()
        fake_data = self.run_G(label)
        ...
        if self.opt.lambda_id != 0:
            i_loss = self.id_loss(syn_img_edited, syn_img)
            i_loss *= self.opt.lambda_id
            self.loss_dict.update(idloss=i_loss)

        # cross loss
        if self.opt.lambda_cross != 0:
            syn_img_pred = self.D(syn_img, 1 - label)  #
            syn_img_edited_pred = self.D(syn_img_edited, 1 - label)
            g_cross_loss = torch.mean(torch.abs(syn_img_pred - syn_img_edited_pred)) * self.opt.lambda_cross
            self.loss_dict.update(cross=g_cross_loss)

        # decouple loss
        if self.opt.lambda_decouple != 0:
            loss1, loss2 = self.G.cal_decouple_loss()
            decouple_loss = (loss1 + loss2).mean()
            decouple_loss = decouple_loss * self.opt.lambda_decouple
            self.loss_dict.update(decouple1=loss1)
            self.loss_dict.update(decouple2=loss2)
        ...
        if self.opt.lambda_decouple != 0:
            gs_loss += decouple_loss

        if self.opt.lambda_cross != 0:
            gs_loss += g_cross_loss
    ...
    def load_networks_fine_tune(self):
        """load the pretrained checkpoints.
            attrs=[attr_name1,attr_name2,atrr_name3]
        """
        ckpt_path1 = './checkpoints/ffhq/%s/010000.pt' % self.opt.attr1
        ckpt_path2 = './checkpoints/ffhq/%s/010000.pt' % self.opt.attr2
        ckpt_path3 = './checkpoints/ffhq/%s/010000.pt' % self.opt.attr3
        ...
        ckpt_g = ckpt1['G']
        ckpt_g['boundary1'] = ckpt1['G']['boundary']
        ckpt_g['boundary2'] = ckpt2['G']['boundary']
        ckpt_g['boundary3'] = ckpt3['G']['boundary']
        ckpt_g.pop('boundary')
        self.G.load_state_dict(ckpt_g)


    def get_visuals(self):
        label = torch.tensor([0, 0, 1, 2])
        fake_data = self.run_G(label)
        syn_img = fake_data['syn_img']
        syn_img_edited = fake_data['syn_img_edited']
        self.visual_dict.update(syn_img=syn_img,
    ...
    def test(self, n_iter=-1):
        ...
        with torch.no_grad():
            ...
            results = dict(l1=l1, l2=l2, fid=0, imgs=imgs)
