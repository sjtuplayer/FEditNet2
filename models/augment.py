...
class AugmentPipe(torch.nn.Module):
    def __init__(self,
        ...
        noise=0, cutout=0, noise_std=0.1, cutout_size=0.5, impl='ref'
    ):
        super().__init__()
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if self.scale > 0:
            s = torch.exp(torch.randn([batch_size], device=device) * self.scale_std * LOG2)
            s = torch.where(torch.rand([batch_size], device=device) < self.scale * self.p, s, torch.ones_like(s))
            if debug_percentile is not None:
                s = torch.full_like(s, torch.exp(torch.erfinv(debug_percentile * 2 - 1) * self.scale_std * LOG2))
        ...
        if self.aniso > 0:
            s = torch.exp(torch.randn([batch_size], device=device) * self.aniso_std * LOG2)
            s = torch.where(torch.rand([batch_size], device=device) < self.aniso * self.p, s, torch.ones_like(s))
            if debug_percentile is not None:
                s = torch.full_like(s, torch.exp(torch.erfinv(debug_percentile * 2 - 1) * self.aniso_std * LOG2))
        ...
        if G_inv is not I_3:
            ...
            # Upsample.
            images = upfirdn2d.upsample2d(x=images, f=self.Hz_geom, up=2, impl=self.impl)
        ...
        if self.contrast > 0:
            c = torch.exp(torch.randn([batch_size], device=device) * self.contrast_std * LOG2)
            c = torch.where(torch.rand([batch_size], device=device) < self.contrast * self.p, c, torch.ones_like(c))
            if debug_percentile is not None:
                c = torch.full_like(c, torch.exp(torch.erfinv(debug_percentile * 2 - 1) * self.contrast_std * LOG2))
            C = scale3d(c, c, c) @ C
        ...
        if self.saturation > 0 and num_channels > 1:
            s = torch.exp(torch.randn([batch_size, 1, 1], device=device) * self.saturation_std * LOG2)
            s = torch.where(torch.rand([batch_size, 1, 1], device=device) < self.saturation * self.p, s, torch.ones_like(s))
            if debug_percentile is not None:
                s = torch.full_like(s, torch.exp(torch.erfinv(debug_percentile * 2 - 1) * self.saturation_std * LOG2))
        ...
        if self.imgfilter > 0:
            ...
            for i, band_strength in enumerate(self.imgfilter_bands):
                t_i = torch.exp(torch.randn([batch_size], device=device) * self.imgfilter_std * LOG2)
                t_i = torch.where(torch.rand([batch_size], device=device) < self.imgfilter * self.p * band_strength, t_i, torch.ones_like(t_i))
                if debug_percentile is not None:
                    t_i = torch.full_like(t_i, torch.exp(torch.erfinv(debug_percentile * 2 - 1) * self.imgfilter_std * LOG2)) if band_strength > 0 else torch.ones_like(t_i)
