...
class BoundaryGenerator2(nn.Module):
    """Generator to train boundary for editing.
    ...
    def __init__(self,
        ...
        Args:
            ...
            fix_len (int)                 --  fixed editing length
        """
        super(BoundaryGenerator2, self).__init__()
        self.latent_dim = latent_dim
        self.init_boundary = init_boundary
        self.fix_len = fix_len
        # use the given `init_boundary` or generate randomly
        boundary1 = F.normalize(torch.randn(1, latent_dim), dim=1) if init_boundary is None else init_boundary
        self.boundary1 = nn.Parameter(boundary1, requires_grad=True)
        boundary2 = F.normalize(torch.randn(1, latent_dim), dim=1) if init_boundary is None else init_boundary
        self.boundary2 = nn.Parameter(boundary2, requires_grad=True)

    def cal_decouple_loss(self):
        sim = torch.abs(torch.cosine_similarity(self.boundary1, self.boundary2))
        return sim
    def forward(self, latent,label, length0=None, cond_boundary=None):
        """Calculate the editing boundary given the latent code.
        ...
        # calculate the editing length
        # when calculating editing length, detach boundary, see TODO above
        latent_edited=torch.zeros_like(latent).to(latent.device)
        for i in range(len(label)):
            if label[i]==0:
                boundary=self.boundary1
            else:
                boundary=self.boundary2
            if length0 is None:
                length = self.fix_len / boundary.detach().norm(dim=1)
            else:
                length = length0 / boundary.detach().norm(dim=1)

            # edited latent code
            latent_edited[i] = latent[i] + boundary * length
...
class BoundaryGenerator3(nn.Module):
    """Generator to train boundary for editing.
    ...
    def __init__(self,
        ...
        Args:
            ...
            fix_len (int)                 --  fixed editing length
        """
        super(BoundaryGenerator3, self).__init__()
        self.latent_dim = latent_dim
        self.init_boundary = init_boundary
        self.fix_len = fix_len
        # use the given `init_boundary` or generate randomly
        ...
        boundary2 = F.normalize(torch.randn(1, latent_dim), dim=1) if init_boundary is None else init_boundary
        self.boundary2 = nn.Parameter(boundary2, requires_grad=True)
        boundary3 = F.normalize(torch.randn(1, latent_dim), dim=1) if init_boundary is None else init_boundary
        self.boundary3 = nn.Parameter(boundary3, requires_grad=True)

    def cal_decouple_loss(self):
        sim1 = torch.abs(torch.cosine_similarity(self.boundary1, self.boundary2))
        sim2 = torch.abs(torch.cosine_similarity(self.boundary1, self.boundary3))
        return sim1, sim2
